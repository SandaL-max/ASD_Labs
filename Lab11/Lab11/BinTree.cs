...
public class BinTree<T> where T : IComparable<T>
{
    ...
    private int GetMinHeight(BinNode<T>? node)
    {
        if (node is null) return 0;
        return Math.Min(GetMinHeight(node.LeftChild), GetMinHeight(node.RightChild)) + 1;
    }
    public bool IdealBalancedTree()
    {
        return GetHeight() - GetMinHeight() <= 1;
    }
    public bool AVLTree()
    {
        return AVLTree(Root);
    }
    private bool AVLTree(BinNode<T>? node)
    {
        if (node is null || (node!.LeftChild is null && node!.RightChild is null))
            return true;
        else if (Math.Abs(GetHeight(node.LeftChild) - GetHeight(node.RightChild)) <= 1)
        {
            return AVLTree(node.LeftChild) && AVLTree(node.RightChild);
        }
        else
            return false;
    }
    ...
    private bool RBTree(BinNode<T>? node)
    {
        if (node is null || (node!.LeftChild is null && node!.RightChild is null))
            return true;
        var Left = GetHeight(node.LeftChild);
        var Right = GetHeight(node.RightChild);
        if (Math.Abs(Left - Right) <= Math.Min(Left, Right))
        {
            return RBTree(node.LeftChild) && RBTree(node.RightChild);
        }
        else
            return false;
    }
